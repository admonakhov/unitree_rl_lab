...
import unitree_rl_lab.tasks.amp.mdp as mdp
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(asset_name="robot", joint_names=[".*"], scale=0.15, use_default_offset=True)
...
class RewardsCfg:
    ...
    undesired_contacts = RewTerm(
        ...
        weight=-1.0,
        params={
                "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[r"^(?!left_ankle_roll_link$)(?!right_ankle_roll_link$)(?!left_wrist_yaw_link$)(?!right_wrist_yaw_link$).+$"]),
                "threshold": 1.0
                }
    )
    # -- optional penalties
